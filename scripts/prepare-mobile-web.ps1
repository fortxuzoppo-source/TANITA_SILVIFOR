...
$source = Join-Path $root "SilviForTANITA"
$target = Join-Path $root "mobile-www"

if (-not (Test-Path -LiteralPath $source)) {
  throw "No encuentro la carpeta web SilviForTANITA."
}
...
if (Test-Path -LiteralPath $target) {
  ...
  if (-not $resolvedTarget.StartsWith($resolvedRoot, [System.StringComparison]::OrdinalIgnoreCase)) {
    throw "La carpeta mobile-www no esta dentro del proyecto."
  }
...
Write-Host "Preparada carpeta movil limpia: $target"
